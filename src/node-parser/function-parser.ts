 ...
 */
export function parseMethodParams(node: FunctionDeclaration | MethodDeclaration | MethodSignature): TshParameter[] {
    return node.parameters.reduce((all: TshParameter[], cur: ParameterDeclaration) => {
        const params = all;
        if (isIdentifier(cur.name)) {
            params.push(
                new TshParameter((cur.name as Identifier).text, getNodeType(cur.type), cur.getStart(), cur.getEnd()),
            );
            ...
            boundParam.parameters = elements.map(
                (bindingElement, index) =>
                    new TshParameter(
            ...
            if (cur.type && isTypeReferenceNode(cur.type)) {
                ...
                // TODO: needs a fix
                // Previous state: types = cur.type.elementTypes.map(type => getNodeType(type));
